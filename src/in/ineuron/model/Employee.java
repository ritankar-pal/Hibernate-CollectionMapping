...
public class Employee implements Serializable {
	...
	//@ElementCollection will create a new table having columns of friendList(name, age, city...etc) class. "EMP_FRNDS" is
	//the name of the new collection table created. This table will have a column named "emp_id" which will have values 
	//from the "eid" field of Employee class (fk).
	
	@ElementCollection
	@Column(name = "friend_name")
	@CollectionTable(name = "EMP_FRNDS", joinColumns = @JoinColumn(name ="emp_id", referencedColumnName = "eid"))
	@OrderColumn(name = "friend_no")
	@ListIndexBase(value = 1)  //List<> is index based that's why
	private List<String> friendList;
	
	@ElementCollection
	@Column(name = "mobile_no")
	@CollectionTable(name = "EMP_PHONE", joinColumns = @JoinColumn(name ="emp_id", referencedColumnName = "eid"))
	private Set<Long> phones;  //No duplicate phone nos
